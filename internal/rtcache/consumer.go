...
func (f *rtConsumer) GetTrip(tid string) (*pb.TripUpdate, bool) {
	log.Trace().Str("feed_id", f.feed).Str("trip", tid).Msg("consumer: get trip")
...
func (f *rtConsumer) Start(ch chan []byte) error {
	log.Trace().Str("feed_id", f.feed).Msg("consumer: start")
	...
	go func() {
		for {
			select {
			case <-f.done:
				log.Trace().Str("feed_id", f.feed).Msg("consumer: done")
				return
			case rtdata := <-ch:
				log.Trace().Str("feed_id", f.feed).Int("bytes", len(rtdata)).Msg("consumer: received data")
	...
	select {
	case <-timeout:
		log.Trace().Str("feed_id", f.feed).Msg("consumer: timed out waiting for first entity")
		return errors.New("timeout waiting for first entity")
	case <-ready:
		log.Trace().Str("feed_id", f.feed).Msg("consumer: ready")
		return nil
	}
}
...
func (f *rtConsumer) process(rtdata []byte) error {
	if len(rtdata) == 0 {
		log.Trace().Str("feed_id", f.feed).Msg("consumer: received no data")
		return nil
	}
	...
	log.Trace().Str("feed_id", f.feed).Int("trip_updates", len(a)).Int("alerts", len(alerts)).Msg("consumer: processed data")
	f.entityByTrip = a
	f.alerts = alerts
	return nil
}
