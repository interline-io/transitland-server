...
func testMeter(t *testing.T, mp MeterProvider, cfg testMeterConfig) {
	t.Run("Meter", func(t *testing.T) {
		...
		v, _ := m.GetValue(cfg.testMeter1)

		m.Meter(cfg.testMeter1, 1, nil)
		mp.Flush()

		a, _ := m.GetValue(cfg.testMeter1)
		assert.Equal(t, 1.0, a-v)

		m.Meter(cfg.testMeter1, 1, nil)
		mp.Flush()

		b, _ := m.GetValue(cfg.testMeter1)
		assert.Equal(t, 2.0, b-v)
	})
	t.Run("NewMeter", func(t *testing.T) {
		m1 := mp.NewMeter(cfg.user1)

		v1, _ := m1.GetValue(cfg.testMeter1)
		v2, _ := m1.GetValue(cfg.testMeter2)

		m1.Meter(cfg.testMeter1, 1, nil)
		m1.Meter(cfg.testMeter2, 2, nil)
		mp.Flush()

		va1, _ := m1.GetValue(cfg.testMeter1)
		assert.Equal(t, 1.0, va1-v1)
		va2, _ := m1.GetValue(cfg.testMeter2)
	...
	t.Run("GetValue", func(t *testing.T) {
		...
		v1, _ := m1.GetValue(cfg.testMeter1)
		v2, _ := m2.GetValue(cfg.testMeter1)
		v3, _ := m3.GetValue(cfg.testMeter1)

		m1.Meter(cfg.testMeter1, 1, nil)
		m2.Meter(cfg.testMeter1, 2.0, nil)
		mp.Flush()

		a, ok := m1.GetValue(cfg.testMeter1)
		assert.Equal(t, 1.0, a-v1)
		assert.Equal(t, true, ok)

		a, ok = m2.GetValue(cfg.testMeter1)
