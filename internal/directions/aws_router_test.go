...
func Test_awsRouter(t *testing.T) {
	tcs := []testCase{
		{"ped", basicTests["ped"], true, 4215, 4.100, testdata.DataPath("fixtures/response/aws_ped.json")},
	...
	for _, tc := range tcs {
		t.Run(tc.name, func(t *testing.T) {
			recorder := testutil.NewRecorder(filepath.Join(testdata.DataPath("fixtures/aws/location"), tc.name), "directions://aws")
			defer recorder.Stop()
