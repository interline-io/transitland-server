...
type CensusValueWhereLoader struct {
	...
	cache map[model.CensusValueParam][]*model.CensusValue

	// the current batch. keys will continue to be collected until timeout is hit,
	// then everything will be sent to the fetch method and out to the listeners
	batch *censusValueWhereLoaderBatch

	// mutex to prevent races
	mu sync.Mutex
}

type censusValueWhereLoaderBatch struct {
	keys    []model.CensusValueParam
	data    [][]*model.CensusValue
	error   []error
	closing bool
	done    chan struct{}
}
...
// LoadThunk returns a function that when called will block waiting for a CensusValue.
...
func (l *CensusValueWhereLoader) LoadThunk(key model.CensusValueParam) func() ([]*model.CensusValue, error) {
	...
	if it, ok := l.cache[key]; ok {
		l.mu.Unlock()
		return func() ([]*model.CensusValue, error) {
			return it, nil
		}
	}
	if l.batch == nil {
		l.batch = &censusValueWhereLoaderBatch{done: make(chan struct{})}
	}
	batch := l.batch
...
func (l *CensusValueWhereLoader) LoadAll(keys []model.CensusValueParam) ([][]*model.CensusValue, []error) {
	results := make([]func() ([]*model.CensusValue, error), len(keys))

	for i, key := range keys {
		results[i] = l.LoadThunk(key)
	}

	censusValues := make([][]*model.CensusValue, len(keys))
	errors := make([]error, len(keys))
	for i, thunk := range results {
		censusValues[i], errors[i] = thunk()
	}
	return censusValues, errors
}

// LoadAllThunk returns a function that when called will block waiting for a CensusValues.
// This method should be used if you want one goroutine to make requests to many
// different data loaders without blocking until the thunk is called.
...
func (l *CensusValueWhereLoader) Prime(key model.CensusValueParam, value []*model.CensusValue) bool {
	...
	if _, found = l.cache[key]; !found {
		...
		cpy := make([]*model.CensusValue, len(value))
		copy(cpy, value)
		l.unsafeSet(key, cpy)
	}
	l.mu.Unlock()
...
func (l *CensusValueWhereLoader) unsafeSet(key model.CensusValueParam, value []*model.CensusValue) {
	if l.cache == nil {
		l.cache = map[model.CensusValueParam][]*model.CensusValue{}
	}
	l.cache[key] = value
}
...
func (b *censusValueWhereLoaderBatch) keyIndex(l *CensusValueWhereLoader, key model.CensusValueParam) int {
...
func (b *censusValueWhereLoaderBatch) startTimer(l *CensusValueWhereLoader) {
	time.Sleep(l.wait)
	l.mu.Lock()
...
func (b *censusValueWhereLoaderBatch) end(l *CensusValueWhereLoader) {
