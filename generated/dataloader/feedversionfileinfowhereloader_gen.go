...
func NewFeedVersionFileInfoWhereLoader(config FeedVersionFileInfoWhereLoaderConfig) *FeedVersionFileInfoWhereLoader {
	return &FeedVersionFileInfoWhereLoader{
		fetch:    config.Fetch,
		wait:     config.Wait,
...
type FeedVersionFileInfoWhereLoader struct {
	...
	cache map[model.FeedVersionFileInfoParam][]*model.FeedVersionFileInfo

	// the current batch. keys will continue to be collected until timeout is hit,
	// then everything will be sent to the fetch method and out to the listeners
	batch *feedVersionFileInfoWhereLoaderBatch

	// mutex to prevent races
	mu sync.Mutex
}

type feedVersionFileInfoWhereLoaderBatch struct {
	keys    []model.FeedVersionFileInfoParam
	data    [][]*model.FeedVersionFileInfo
	error   []error
	closing bool
	done    chan struct{}
}
...
// LoadThunk returns a function that when called will block waiting for a FeedVersionFileInfo.
...
func (l *FeedVersionFileInfoWhereLoader) LoadThunk(key model.FeedVersionFileInfoParam) func() ([]*model.FeedVersionFileInfo, error) {
	...
	if it, ok := l.cache[key]; ok {
		l.mu.Unlock()
		return func() ([]*model.FeedVersionFileInfo, error) {
			return it, nil
		}
	}
	if l.batch == nil {
		l.batch = &feedVersionFileInfoWhereLoaderBatch{done: make(chan struct{})}
	}
	batch := l.batch
	...
	return func() ([]*model.FeedVersionFileInfo, error) {
		...
		var data []*model.FeedVersionFileInfo
...
func (l *FeedVersionFileInfoWhereLoader) LoadAll(keys []model.FeedVersionFileInfoParam) ([][]*model.FeedVersionFileInfo, []error) {
	results := make([]func() ([]*model.FeedVersionFileInfo, error), len(keys))

	for i, key := range keys {
		results[i] = l.LoadThunk(key)
	}

	feedVersionFileInfos := make([][]*model.FeedVersionFileInfo, len(keys))
	errors := make([]error, len(keys))
	for i, thunk := range results {
		feedVersionFileInfos[i], errors[i] = thunk()
	}
	return feedVersionFileInfos, errors
}

// LoadAllThunk returns a function that when called will block waiting for a FeedVersionFileInfos.
// This method should be used if you want one goroutine to make requests to many
// different data loaders without blocking until the thunk is called.
...
func (l *FeedVersionFileInfoWhereLoader) Prime(key model.FeedVersionFileInfoParam, value []*model.FeedVersionFileInfo) bool {
	...
	if _, found = l.cache[key]; !found {
		...
		cpy := make([]*model.FeedVersionFileInfo, len(value))
		copy(cpy, value)
		l.unsafeSet(key, cpy)
	}
	l.mu.Unlock()
...
func (l *FeedVersionFileInfoWhereLoader) unsafeSet(key model.FeedVersionFileInfoParam, value []*model.FeedVersionFileInfo) {
	if l.cache == nil {
		l.cache = map[model.FeedVersionFileInfoParam][]*model.FeedVersionFileInfo{}
	}
	l.cache[key] = value
}
...
func (b *feedVersionFileInfoWhereLoaderBatch) keyIndex(l *FeedVersionFileInfoWhereLoader, key model.FeedVersionFileInfoParam) int {
...
func (b *feedVersionFileInfoWhereLoaderBatch) startTimer(l *FeedVersionFileInfoWhereLoader) {
	time.Sleep(l.wait)
	l.mu.Lock()
...
func (b *feedVersionFileInfoWhereLoaderBatch) end(l *FeedVersionFileInfoWhereLoader) {
