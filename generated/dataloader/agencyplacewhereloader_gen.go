...
type AgencyPlaceWhereLoader struct {
	...
	cache map[model.AgencyPlaceParam][]*model.AgencyPlace

	// the current batch. keys will continue to be collected until timeout is hit,
	// then everything will be sent to the fetch method and out to the listeners
	batch *agencyPlaceWhereLoaderBatch

	// mutex to prevent races
	mu sync.Mutex
}

type agencyPlaceWhereLoaderBatch struct {
	keys    []model.AgencyPlaceParam
	data    [][]*model.AgencyPlace
	error   []error
	closing bool
	done    chan struct{}
}
...
// LoadThunk returns a function that when called will block waiting for a AgencyPlace.
...
func (l *AgencyPlaceWhereLoader) LoadThunk(key model.AgencyPlaceParam) func() ([]*model.AgencyPlace, error) {
	...
	if it, ok := l.cache[key]; ok {
		l.mu.Unlock()
		return func() ([]*model.AgencyPlace, error) {
			return it, nil
		}
	}
	if l.batch == nil {
		l.batch = &agencyPlaceWhereLoaderBatch{done: make(chan struct{})}
	}
	batch := l.batch
...
func (l *AgencyPlaceWhereLoader) LoadAll(keys []model.AgencyPlaceParam) ([][]*model.AgencyPlace, []error) {
	results := make([]func() ([]*model.AgencyPlace, error), len(keys))

	for i, key := range keys {
		results[i] = l.LoadThunk(key)
	}

	agencyPlaces := make([][]*model.AgencyPlace, len(keys))
	errors := make([]error, len(keys))
	for i, thunk := range results {
		agencyPlaces[i], errors[i] = thunk()
	}
	return agencyPlaces, errors
}

// LoadAllThunk returns a function that when called will block waiting for a AgencyPlaces.
// This method should be used if you want one goroutine to make requests to many
// different data loaders without blocking until the thunk is called.
...
func (l *AgencyPlaceWhereLoader) Prime(key model.AgencyPlaceParam, value []*model.AgencyPlace) bool {
	...
	if _, found = l.cache[key]; !found {
		...
		cpy := make([]*model.AgencyPlace, len(value))
		copy(cpy, value)
		l.unsafeSet(key, cpy)
	}
	l.mu.Unlock()
...
func (l *AgencyPlaceWhereLoader) unsafeSet(key model.AgencyPlaceParam, value []*model.AgencyPlace) {
	if l.cache == nil {
		l.cache = map[model.AgencyPlaceParam][]*model.AgencyPlace{}
	}
	l.cache[key] = value
}
...
func (b *agencyPlaceWhereLoaderBatch) keyIndex(l *AgencyPlaceWhereLoader, key model.AgencyPlaceParam) int {
...
func (b *agencyPlaceWhereLoaderBatch) startTimer(l *AgencyPlaceWhereLoader) {
	time.Sleep(l.wait)
	l.mu.Lock()
...
func (b *agencyPlaceWhereLoaderBatch) end(l *AgencyPlaceWhereLoader) {
