...
func NewFeedVersionServiceLevelWhereLoader(config FeedVersionServiceLevelWhereLoaderConfig) *FeedVersionServiceLevelWhereLoader {
	return &FeedVersionServiceLevelWhereLoader{
		fetch:    config.Fetch,
		wait:     config.Wait,
...
type FeedVersionServiceLevelWhereLoader struct {
	...
	cache map[model.FeedVersionServiceLevelParam][]*model.FeedVersionServiceLevel

	// the current batch. keys will continue to be collected until timeout is hit,
	// then everything will be sent to the fetch method and out to the listeners
	batch *feedVersionServiceLevelWhereLoaderBatch

	// mutex to prevent races
	mu sync.Mutex
}

type feedVersionServiceLevelWhereLoaderBatch struct {
	keys    []model.FeedVersionServiceLevelParam
	data    [][]*model.FeedVersionServiceLevel
	error   []error
	closing bool
	done    chan struct{}
}
...
// LoadThunk returns a function that when called will block waiting for a FeedVersionServiceLevel.
...
func (l *FeedVersionServiceLevelWhereLoader) LoadThunk(key model.FeedVersionServiceLevelParam) func() ([]*model.FeedVersionServiceLevel, error) {
	...
	if it, ok := l.cache[key]; ok {
		l.mu.Unlock()
		return func() ([]*model.FeedVersionServiceLevel, error) {
			return it, nil
		}
	}
	if l.batch == nil {
		l.batch = &feedVersionServiceLevelWhereLoaderBatch{done: make(chan struct{})}
	}
	batch := l.batch
	...
	return func() ([]*model.FeedVersionServiceLevel, error) {
		...
		var data []*model.FeedVersionServiceLevel
...
func (l *FeedVersionServiceLevelWhereLoader) LoadAll(keys []model.FeedVersionServiceLevelParam) ([][]*model.FeedVersionServiceLevel, []error) {
	results := make([]func() ([]*model.FeedVersionServiceLevel, error), len(keys))

	for i, key := range keys {
		results[i] = l.LoadThunk(key)
	}

	feedVersionServiceLevels := make([][]*model.FeedVersionServiceLevel, len(keys))
	errors := make([]error, len(keys))
	for i, thunk := range results {
		feedVersionServiceLevels[i], errors[i] = thunk()
	}
	return feedVersionServiceLevels, errors
}

// LoadAllThunk returns a function that when called will block waiting for a FeedVersionServiceLevels.
// This method should be used if you want one goroutine to make requests to many
// different data loaders without blocking until the thunk is called.
...
func (l *FeedVersionServiceLevelWhereLoader) Prime(key model.FeedVersionServiceLevelParam, value []*model.FeedVersionServiceLevel) bool {
	...
	if _, found = l.cache[key]; !found {
		...
		cpy := make([]*model.FeedVersionServiceLevel, len(value))
		copy(cpy, value)
		l.unsafeSet(key, cpy)
	}
	l.mu.Unlock()
...
func (l *FeedVersionServiceLevelWhereLoader) unsafeSet(key model.FeedVersionServiceLevelParam, value []*model.FeedVersionServiceLevel) {
	if l.cache == nil {
		l.cache = map[model.FeedVersionServiceLevelParam][]*model.FeedVersionServiceLevel{}
	}
	l.cache[key] = value
}
...
func (b *feedVersionServiceLevelWhereLoaderBatch) keyIndex(l *FeedVersionServiceLevelWhereLoader, key model.FeedVersionServiceLevelParam) int {
...
func (b *feedVersionServiceLevelWhereLoaderBatch) startTimer(l *FeedVersionServiceLevelWhereLoader) {
	time.Sleep(l.wait)
	l.mu.Lock()
...
func (b *feedVersionServiceLevelWhereLoaderBatch) end(l *FeedVersionServiceLevelWhereLoader) {
