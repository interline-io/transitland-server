...
type CalendarLoader struct {
	...
	cache map[int]*model.Calendar

	// the current batch. keys will continue to be collected until timeout is hit,
	// then everything will be sent to the fetch method and out to the listeners
	batch *calendarLoaderBatch

	// mutex to prevent races
	mu sync.Mutex
}

type calendarLoaderBatch struct {
	keys    []int
	data    []*model.Calendar
	error   []error
	closing bool
...
// LoadThunk returns a function that when called will block waiting for a Calendar.
...
func (l *CalendarLoader) LoadThunk(key int) func() (*model.Calendar, error) {
	...
	if it, ok := l.cache[key]; ok {
		l.mu.Unlock()
		return func() (*model.Calendar, error) {
			return it, nil
		}
	}
	if l.batch == nil {
		l.batch = &calendarLoaderBatch{done: make(chan struct{})}
	}
	batch := l.batch
...
func (l *CalendarLoader) LoadAll(keys []int) ([]*model.Calendar, []error) {
	results := make([]func() (*model.Calendar, error), len(keys))

	for i, key := range keys {
		results[i] = l.LoadThunk(key)
	}

	calendars := make([]*model.Calendar, len(keys))
	errors := make([]error, len(keys))
	for i, thunk := range results {
		calendars[i], errors[i] = thunk()
	}
	return calendars, errors
}

// LoadAllThunk returns a function that when called will block waiting for a Calendars.
// This method should be used if you want one goroutine to make requests to many
// different data loaders without blocking until the thunk is called.
...
func (l *CalendarLoader) unsafeSet(key int, value *model.Calendar) {
	if l.cache == nil {
		l.cache = map[int]*model.Calendar{}
	}
	l.cache[key] = value
}
...
func (b *calendarLoaderBatch) keyIndex(l *CalendarLoader, key int) int {
...
func (b *calendarLoaderBatch) startTimer(l *CalendarLoader) {
	time.Sleep(l.wait)
	l.mu.Lock()
...
func (b *calendarLoaderBatch) end(l *CalendarLoader) {
