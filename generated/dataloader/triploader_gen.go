...
type TripLoader struct {
	...
	cache map[int]*model.Trip

	// the current batch. keys will continue to be collected until timeout is hit,
	// then everything will be sent to the fetch method and out to the listeners
	batch *tripLoaderBatch

	// mutex to prevent races
	mu sync.Mutex
}

type tripLoaderBatch struct {
	keys    []int
	data    []*model.Trip
	error   []error
	closing bool
...
func (l *TripLoader) LoadThunk(key int) func() (*model.Trip, error) {
	...
	if l.batch == nil {
		l.batch = &tripLoaderBatch{done: make(chan struct{})}
	}
	batch := l.batch
...
func (l *TripLoader) unsafeSet(key int, value *model.Trip) {
	if l.cache == nil {
		l.cache = map[int]*model.Trip{}
	}
	l.cache[key] = value
}
...
func (b *tripLoaderBatch) keyIndex(l *TripLoader, key int) int {
...
func (b *tripLoaderBatch) startTimer(l *TripLoader) {
	time.Sleep(l.wait)
	l.mu.Lock()
...
func (b *tripLoaderBatch) end(l *TripLoader) {
