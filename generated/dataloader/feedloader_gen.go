...
type FeedLoader struct {
	...
	cache map[int]*model.Feed

	// the current batch. keys will continue to be collected until timeout is hit,
	// then everything will be sent to the fetch method and out to the listeners
	batch *feedLoaderBatch

	// mutex to prevent races
	mu sync.Mutex
}

type feedLoaderBatch struct {
	keys    []int
	data    []*model.Feed
	error   []error
	closing bool
...
func (l *FeedLoader) LoadThunk(key int) func() (*model.Feed, error) {
	...
	if l.batch == nil {
		l.batch = &feedLoaderBatch{done: make(chan struct{})}
	}
	batch := l.batch
...
func (l *FeedLoader) unsafeSet(key int, value *model.Feed) {
	if l.cache == nil {
		l.cache = map[int]*model.Feed{}
	}
	l.cache[key] = value
}
...
func (b *feedLoaderBatch) keyIndex(l *FeedLoader, key int) int {
...
func (b *feedLoaderBatch) startTimer(l *FeedLoader) {
	time.Sleep(l.wait)
	l.mu.Lock()
...
func (b *feedLoaderBatch) end(l *FeedLoader) {
