...
type FrequencyWhereLoader struct {
	...
	cache map[model.FrequencyParam][]*model.Frequency

	// the current batch. keys will continue to be collected until timeout is hit,
	// then everything will be sent to the fetch method and out to the listeners
	batch *frequencyWhereLoaderBatch

	// mutex to prevent races
	mu sync.Mutex
}

type frequencyWhereLoaderBatch struct {
	keys    []model.FrequencyParam
...
// LoadThunk returns a function that when called will block waiting for a Frequency.
...
func (l *FrequencyWhereLoader) LoadThunk(key model.FrequencyParam) func() ([]*model.Frequency, error) {
	...
	if it, ok := l.cache[key]; ok {
		l.mu.Unlock()
		return func() ([]*model.Frequency, error) {
			return it, nil
		}
	}
	if l.batch == nil {
		l.batch = &frequencyWhereLoaderBatch{done: make(chan struct{})}
	}
	batch := l.batch
...
func (l *FrequencyWhereLoader) LoadAll(keys []model.FrequencyParam) ([][]*model.Frequency, []error) {
	results := make([]func() ([]*model.Frequency, error), len(keys))

	for i, key := range keys {
		results[i] = l.LoadThunk(key)
	}

	frequencys := make([][]*model.Frequency, len(keys))
	errors := make([]error, len(keys))
	for i, thunk := range results {
		frequencys[i], errors[i] = thunk()
	}
	return frequencys, errors
}

// LoadAllThunk returns a function that when called will block waiting for a Frequencys.
// This method should be used if you want one goroutine to make requests to many
// different data loaders without blocking until the thunk is called.
...
func (l *FrequencyWhereLoader) Prime(key model.FrequencyParam, value []*model.Frequency) bool {
	...
	if _, found = l.cache[key]; !found {
		...
		cpy := make([]*model.Frequency, len(value))
		copy(cpy, value)
		l.unsafeSet(key, cpy)
	}
	l.mu.Unlock()
...
func (l *FrequencyWhereLoader) unsafeSet(key model.FrequencyParam, value []*model.Frequency) {
	if l.cache == nil {
		l.cache = map[model.FrequencyParam][]*model.Frequency{}
	}
	l.cache[key] = value
}
...
func (b *frequencyWhereLoaderBatch) keyIndex(l *FrequencyWhereLoader, key model.FrequencyParam) int {
...
func (b *frequencyWhereLoaderBatch) startTimer(l *FrequencyWhereLoader) {
	time.Sleep(l.wait)
	l.mu.Lock()
...
func (b *frequencyWhereLoaderBatch) end(l *FrequencyWhereLoader) {
