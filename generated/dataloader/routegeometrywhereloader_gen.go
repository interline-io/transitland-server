...
func NewRouteGeometryWhereLoader(config RouteGeometryWhereLoaderConfig) *RouteGeometryWhereLoader {
	return &RouteGeometryWhereLoader{
		fetch:    config.Fetch,
		wait:     config.Wait,
...
type RouteGeometryWhereLoader struct {
	...
	cache map[model.RouteGeometryParam][]*model.RouteGeometry

	// the current batch. keys will continue to be collected until timeout is hit,
	// then everything will be sent to the fetch method and out to the listeners
	batch *routeGeometryWhereLoaderBatch

	// mutex to prevent races
	mu sync.Mutex
}

type routeGeometryWhereLoaderBatch struct {
	keys    []model.RouteGeometryParam
	data    [][]*model.RouteGeometry
	error   []error
	closing bool
	done    chan struct{}
}
...
// LoadThunk returns a function that when called will block waiting for a RouteGeometry.
...
func (l *RouteGeometryWhereLoader) LoadThunk(key model.RouteGeometryParam) func() ([]*model.RouteGeometry, error) {
	...
	if it, ok := l.cache[key]; ok {
		l.mu.Unlock()
		return func() ([]*model.RouteGeometry, error) {
			return it, nil
		}
	}
	if l.batch == nil {
		l.batch = &routeGeometryWhereLoaderBatch{done: make(chan struct{})}
	}
	batch := l.batch
...
func (l *RouteGeometryWhereLoader) LoadAll(keys []model.RouteGeometryParam) ([][]*model.RouteGeometry, []error) {
	results := make([]func() ([]*model.RouteGeometry, error), len(keys))

	for i, key := range keys {
		results[i] = l.LoadThunk(key)
	}

	routeGeometrys := make([][]*model.RouteGeometry, len(keys))
	errors := make([]error, len(keys))
	for i, thunk := range results {
		routeGeometrys[i], errors[i] = thunk()
	}
	return routeGeometrys, errors
}

// LoadAllThunk returns a function that when called will block waiting for a RouteGeometrys.
// This method should be used if you want one goroutine to make requests to many
// different data loaders without blocking until the thunk is called.
...
func (l *RouteGeometryWhereLoader) Prime(key model.RouteGeometryParam, value []*model.RouteGeometry) bool {
	...
	if _, found = l.cache[key]; !found {
		...
		cpy := make([]*model.RouteGeometry, len(value))
		copy(cpy, value)
		l.unsafeSet(key, cpy)
	}
	l.mu.Unlock()
...
func (l *RouteGeometryWhereLoader) unsafeSet(key model.RouteGeometryParam, value []*model.RouteGeometry) {
	if l.cache == nil {
		l.cache = map[model.RouteGeometryParam][]*model.RouteGeometry{}
	}
	l.cache[key] = value
}
...
func (b *routeGeometryWhereLoaderBatch) keyIndex(l *RouteGeometryWhereLoader, key model.RouteGeometryParam) int {
...
func (b *routeGeometryWhereLoaderBatch) startTimer(l *RouteGeometryWhereLoader) {
	time.Sleep(l.wait)
	l.mu.Lock()
...
func (b *routeGeometryWhereLoaderBatch) end(l *RouteGeometryWhereLoader) {
