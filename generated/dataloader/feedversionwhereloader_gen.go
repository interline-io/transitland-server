...
type FeedVersionWhereLoader struct {
	...
	cache map[model.FeedVersionParam][]*model.FeedVersion

	// the current batch. keys will continue to be collected until timeout is hit,
	// then everything will be sent to the fetch method and out to the listeners
	batch *feedVersionWhereLoaderBatch

	// mutex to prevent races
	mu sync.Mutex
}

type feedVersionWhereLoaderBatch struct {
	keys    []model.FeedVersionParam
...
func (l *FeedVersionWhereLoader) LoadThunk(key model.FeedVersionParam) func() ([]*model.FeedVersion, error) {
	...
	if l.batch == nil {
		l.batch = &feedVersionWhereLoaderBatch{done: make(chan struct{})}
	}
	batch := l.batch
...
func (l *FeedVersionWhereLoader) Prime(key model.FeedVersionParam, value []*model.FeedVersion) bool {
	...
	if _, found = l.cache[key]; !found {
		...
		cpy := make([]*model.FeedVersion, len(value))
		copy(cpy, value)
		l.unsafeSet(key, cpy)
	}
	l.mu.Unlock()
...
func (l *FeedVersionWhereLoader) unsafeSet(key model.FeedVersionParam, value []*model.FeedVersion) {
	if l.cache == nil {
		l.cache = map[model.FeedVersionParam][]*model.FeedVersion{}
	}
	l.cache[key] = value
}
...
func (b *feedVersionWhereLoaderBatch) keyIndex(l *FeedVersionWhereLoader, key model.FeedVersionParam) int {
...
func (b *feedVersionWhereLoaderBatch) startTimer(l *FeedVersionWhereLoader) {
	time.Sleep(l.wait)
	l.mu.Lock()
...
func (b *feedVersionWhereLoaderBatch) end(l *FeedVersionWhereLoader) {
