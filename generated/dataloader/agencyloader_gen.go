...
type AgencyLoader struct {
	...
	cache map[int]*model.Agency

	// the current batch. keys will continue to be collected until timeout is hit,
	// then everything will be sent to the fetch method and out to the listeners
	batch *agencyLoaderBatch

	// mutex to prevent races
	mu sync.Mutex
}

type agencyLoaderBatch struct {
	keys    []int
	data    []*model.Agency
	error   []error
	closing bool
...
func (l *AgencyLoader) LoadThunk(key int) func() (*model.Agency, error) {
	...
	if l.batch == nil {
		l.batch = &agencyLoaderBatch{done: make(chan struct{})}
	}
	batch := l.batch
...
func (l *AgencyLoader) unsafeSet(key int, value *model.Agency) {
	if l.cache == nil {
		l.cache = map[int]*model.Agency{}
	}
	l.cache[key] = value
}
...
func (b *agencyLoaderBatch) keyIndex(l *AgencyLoader, key int) int {
...
func (b *agencyLoaderBatch) startTimer(l *AgencyLoader) {
	time.Sleep(l.wait)
	l.mu.Lock()
...
func (b *agencyLoaderBatch) end(l *AgencyLoader) {
