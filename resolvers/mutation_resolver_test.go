...
func TestValidationResolver(t *testing.T) {
	...
	srv, _ := NewServer(config.Config{DB: dbCfg})
	srv = auth.UserDefaultMiddleware(nil)(srv) // Run all requests as user
	c := client.New(srv)
	...
	t.Run("requires user access", func(t *testing.T) {
		srv, _ := NewServer(config.Config{DB: dbCfg}) // all requests run as anonymous context by default
		c := client.New(srv)
		resp := make(map[string]interface{})
